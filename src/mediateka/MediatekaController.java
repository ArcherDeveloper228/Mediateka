...
public class MediatekaController {
	...
	private void initialize() {
		...
		this.button_update.setOnAction(event -> {
			...
			// устанавливаем кнопку добавлени€ доступной
			this.button_add.setDisable(false);
			// утсанавливаем кнопку обновлени€ информации недоступной
			this.button_update.setDisable(true);

			// если есть информаци€, которую можно загрузить с сервера
			if (files != null) {
	...
	private final String getFileExtension(String file_name) {
        ...
        // если в имени файла есть точка и она не €вл€етс€ первым символом в названии файла
        if(file_name.lastIndexOf(".") != -1 && file_name.lastIndexOf(".") != 0)
        // то вырезаем все знаки после последней точки в названии файла, то есть ’’’’’.txt -> txt
